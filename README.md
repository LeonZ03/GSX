...
先核清相机、照片边界和独立复核视角能否支撑验收，再按完整部件组推进。暂按**10–14个大工作轮**安排剩余工作：参考基线1轮、主要覆盖件3–4轮、机械驾驶位附件3–4轮、版画1–2轮、材质UV1轮、交付1–2轮。它是低可信度的管理预算，**不是保证再若干版达到1:1**；第一轮结束后必须按证据重估。

前4–5大轮必须交出整车灰模验收结果，或明确的未通过原因和路线调整。每轮内部连续试验；同一问题连续两轮没有实质改善就换方法，达到阶段预算不静默顺延、不降低标准。r编号仅用于追溯。

62/63/69均参与过形体拟合，64欠约束，70失败；两个有效独立角度和完整分件遮罩仍缺。先解决可验收性，再继续油箱/座尾、车头/侧罩；灰模通过后进入机械、版画与最终交付。[具体计划与退出条件](reconstruction_v2/ITERATION_PLAN.md) · [问题清单](reconstruction_v2/ISSUES.md)。
