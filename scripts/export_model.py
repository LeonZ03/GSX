...
report={'source_version':bpy.app.version_string,'objects':len(mapping),'vertices':sum(len(o.data.vertices) for o,n in mapping),'polygons':sum(len(o.data.polygons) for o,n in mapping),'uv_missing':sum(not o.data.uv_layers for o,n in mapping),'materials_missing':sum(not len(o.data.materials) for o,n in mapping),'empty_objects_removed':bad,'formats':{}}
