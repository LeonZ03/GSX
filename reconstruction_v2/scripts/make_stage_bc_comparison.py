...
for k in [62,63,69]:
 ...
 before=Image.open(V/f'renders/stage_bc_baseline/gray_r15_{k}.png').convert('RGBA').resize(photo.size,Image.Resampling.LANCZOS)
 ...
 for i,(im,label) in enumerate(zip(frames,[f'实车 {k}（隐私遮蔽）','r15 中性灰基线',f'{TAG} 结构重建'])):
