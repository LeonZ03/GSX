...
def masked(im,ann):
 # Opaque hatching makes exclusion regions visibly part of the 2D review UI.
 # Never reveal underlying photo pixels, especially in privacy regions.
 im=im.copy()
 names={'luggage':'尾包排除区','privacy':'隐私遮蔽','license_plate':'隐私遮蔽','net_and_luggage_state':'遮挡排除区'}
 for reg in ann.get('exclude_polygons',[]):
  if not reg.get('polygon'):continue
  poly=[tuple(p) for p in reg['polygon']];mask=Image.new('L',im.size,0);ImageDraw.Draw(mask).polygon(poly,fill=255)
  fill=Image.new('RGB',im.size,(221,225,230));d=ImageDraw.Draw(fill)
  for x in range(-im.height,im.width,24):d.line((x,0,x+im.height,im.height),fill=(169,178,189),width=2)
  x0=min(p[0] for p in poly);x1=max(p[0] for p in poly);y0=min(p[1] for p in poly);y1=max(p[1] for p in poly)
  label=names.get(reg.get('label'),'排除区');box=d.textbbox((0,0),label,font=FONT);tw=box[2]-box[0]
  pos=((x0+x1-tw)/2,(y0+y1)/2-14)
  d.rectangle((pos[0]-5,pos[1]-2,pos[0]+tw+5,pos[1]+30),fill=(238,240,243));d.text(pos,label,font=FONT,fill=(55,65,79))
  im.paste(fill,(0,0),mask)
 return im

def main():
 boards=[];report={'revision':TAG,'gate':'NOT_PASSED','scope':'neutral gray evidence review','display_revision':'unmasked_model_panel_labeled_hatching','exclusion_policy':'Fixed exclusion polygons retained for comparison; pure gray panel shows the full model without any image overlay.','silhouette_iou':None,'silhouette_iou_reason':'Full closed foreground and part masks have not passed annotation review. Observed open contours are visual diagnostics only.','views':{}}
 ...
 for k in ids:
  ...
  ref=masked(photo,ann);neutral=Image.new('RGB',photo.size,(228,228,228));neutral.paste(gray,(0,0),gray)
  ...
  labels=['原照片（斜线为排除区）','完整灰模（无尾包／无遮罩）','半透明叠加（斜线为排除区）','轮廓对照：青＝模型；橙＝已标注实物边缘']
