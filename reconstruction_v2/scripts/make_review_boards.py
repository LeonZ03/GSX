...
if TAG not in ['r01','r02','r03','r04']:raise ValueError('Unknown review revision')
...
def main():
 ...
 for k in [62,63,64]:
  ...
  for line in EDGES[k]:cv2.polylines(diff,[np.array(line,np.int32)],False,(242,116,33),4)
  ...
  board=Image.new('RGB',(1280,1848),'white');d=ImageDraw.Draw(board)
  labels=['原照片（固定排除区已遮蔽）','同相机灰模 / Cycles','半透明叠加','轮廓对照：青＝模型；橙＝已标注实物边缘']
  for i,im in enumerate([ref,neutral,over,diff]):
   im=im.resize((640,853));x=(i%2)*640;y=(i//2)*910+43;board.paste(im,(x,y));d.text((x+12,y-32),labels[i],font=SMALL,fill=(30,30,30))
  d.text((12,1815),f'照片 {k} · {TAG} · 外形未通过 · '+('独立角度，但镜头仍欠约束' if k==64 else '镜头参数已冻结供本轮对照；仍非实测标定'),font=SMALL,fill=(160,38,28))
 ...
 contact=Image.new('RGB',(1536,740),'white')
 for i,b in enumerate(boards):contact.paste(b.resize((512,739)),(i*512,0))
 ...
 report['reserved_unfit_views']=[61,66];report['heldout_gate']='NOT_PASSED: two reliable independent cameras are still required'
 ...
 print('Created 3 four-panel boards and contact sheet; no unverified IoU reported.')
