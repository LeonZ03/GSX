"""Three frozen-camera native crops for r13 interface review; never save scene."""
from pathlib import Path
import bpy,json,math,sys
ROOT=Path(__file__).resolve().parents[2];V2=ROOT/'reconstruction_v2';s=bpy.context.scene;tag=s.get('revision');out=V2/'renders/interface_r13';out.mkdir(exist_ok=True);ann=json.loads((V2/'annotations/tank_interface_r13.json').read_text())
s.render.engine='CYCLES';s.cycles.samples=32;s.cycles.use_denoising=True;s.render.film_transparent=True;s.render.resolution_percentage=100;s.render.use_border=True;s.render.use_crop_to_border=True;s.render.image_settings.file_format='PNG';s.render.image_settings.color_mode='RGBA'
try:
