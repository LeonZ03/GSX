...
z=np.load(V2/'qa/tank_full_basis_r15.npz');v=z['vertices'];b=z['basis'];meta=json.loads((V2/'qa/tank_full_basis_r15.json').read_text());ann=json.loads((V2/'annotations/tank_crown_r14.json').read_text());views=[]
for k in ['62','63','69']:
...
if (V2/'qa/tank_full_fit_r15.json').exists():raise FileExistsError('Preserve completed r14 candidate run; choose a new revision')
...
for side_weight in [2,3]:
 ...
 def residual(q):
  ...
  res.extend(q*penalty);res.extend((np.diff(q.reshape(10,4),n=2,axis=0)*.35).ravel());g=z['control_grid']+np.einsum('k,kijc->ijc',q,z['control_basis']);res.extend(np.minimum(np.diff(g[:,:,1],axis=0)-4,0).ravel()*2)
  return np.array(res)
 initial=np.r_[list(json.loads((V2/'qa/tank_crown_fit_r14.json').read_text())[1]['params'].values()),np.zeros(4)]
 fit=least_squares(residual,initial,bounds=(z['lower'],z['upper']),diff_step=.005,loss='soft_l1',f_scale=4,max_nfev=180,ftol=1e-6,xtol=1e-6);q=fit.x;name=f'full_joint_w{side_weight}_r15';cage=dict(meta['cage_template']);cage['grid']=(z['control_grid']+np.einsum('k,kijc->ijc',q,z['control_basis'])).tolist();cage['revision']='r15';cage['revision_note']='Separate rear crown/width/knee controls fitted with fixed camera rear envelopes; full-stack review required.'
...
(V2/'qa/tank_full_fit_r15.json').write_text(json.dumps(rows,indent=2),encoding='utf8')
