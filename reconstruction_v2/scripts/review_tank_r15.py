...
R15 uses three fitting views, not independent validation. Open boundary distance
...
before='r13';after=sys.argv[1] if len(sys.argv)>1 else 'r15';ann=json.loads((V2/'annotations/tank_crown_r14.json').read_text());out=V2/'renders/interface_r14';font=ImageFont.truetype('C:/Windows/Fonts/msyh.ttc',26);small=ImageFont.truetype('C:/Windows/Fonts/msyh.ttc',19)
board=Image.new('RGB',(1800,1280),'#f6f8fa');draw=ImageDraw.Draw(board);draw.text((18,12),'r15 油箱连续重修：双上缘约束、前后收口与体量',font=font,fill='#213547');draw.text((18,52),'相机固定；62 / 63 / 69均参与拟合。灰模未通过；照片只保存在本地。',font=small,fill='#874331');metrics={};origins={}
def labels(im):
...
for row,(key,a) in enumerate(ann['views'].items()):
 ...
 for col,(im,title) in enumerate(zip(panels,['实车局部','改前 r13','改后 r15','黄=参考  紫=改前  青=改后'])):
...
board.save(V2/'renders/tank_crown_r15_comparison.jpg',quality=94)
...
(V2/'qa/tank_review_r15.json').write_text(json.dumps(report,indent=2),encoding='utf8');print(json.dumps({k:{tag:{kind:round(v['mean_px'],2) for kind,v in m.items()} for tag,m in val.items()} for k,val in metrics.items()},indent=2))
